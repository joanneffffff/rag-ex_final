...
# AlphaFin中文编码器微调脚本
# 基于现有的finetune_chinese_encoder.py进行优化

echo "=========================================="
echo "AlphaFin中文编码器微调脚本"
echo "=========================================="
...
# 数据路径配置
TRAIN_DATA="evaluate_mrr/alphafin_train_qc_enhanced.jsonl"
EVAL_DATA="evaluate_mrr/alphafin_eval_enhanced.jsonl"
RAW_DATA="data/alphafin/alphafin_cleaned.json"

# 模型配置
BASE_MODEL="Langboat/mengzi-bert-base-fin"
OUTPUT_MODEL_PATH="./models/finetuned_alphafin_encoder"
...
# 开始微调
echo "开始AlphaFin编码器微调..."
echo "时间: $(date)"
echo ""

python encoder_finetune_evaluate/finetune_chinese_encoder.py \
    --base_encoder_model_name "$BASE_MODEL" \
    ...
    --limit_train_data $LIMIT_TRAIN \
    --limit_eval_data $LIMIT_EVAL \
...
if [ $? -eq 0 ]; then
    ...
    echo "       --base_raw_data_path $RAW_DATA"
else
    echo ""
