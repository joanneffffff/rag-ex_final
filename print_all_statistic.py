...
def analyze_alphafin_processed_data():
    ...
    try:
        ...
        # 检查不同的问题字段
        question_fields = ['generated_question', 'question', 'original_question', 'query']
        question_field_stats = {}
        
        for field in question_fields:
            count = sum(1 for item in processed_data 
                       if item.get(field) and str(item.get(field)).strip())
            percentage = count / len(processed_data) * 100
            question_field_stats[field] = (count, percentage)
            print(f"   {field}: {count:,}/{len(processed_data):,} ({percentage:.1f}%)")
        ...
        print(f"   无任何问题字段的记录: {len(empty_questions):,} ({len(empty_questions)/len(processed_data)*100:.1f}%)")
        
        if empty_questions:
            print(f"   前5个空问题记录的索引: {empty_questions[:5]}")
            # 显示一个空问题记录的示例
            if empty_questions:
                sample_empty = processed_data[empty_questions[0]]
                print(f"   空问题记录示例 (索引 {empty_questions[0]}):")
                print(f"     字段: {list(sample_empty.keys())}")
                for field in question_fields:
                    value = sample_empty.get(field, 'NOT_FOUND')
                    print(f"     {field}: '{value}'")
        
        # 长度统计（使用主要问题字段）
        primary_question_field = 'generated_question'  # 主要使用这个字段
        ...
        question_lengths = [len(item.get(primary_question_field, '')) for item in processed_data]
        
        # 过滤掉空问题
        valid_question_lengths = [length for length in question_lengths if length > 0]
        
        print(f"\n📏 长度统计:")
        print(f"   Context平均长度: {statistics.mean(context_lengths):.1f} 字符")
        print(f"   Answer平均长度: {statistics.mean(answer_lengths):.1f} 字符")
        print(f"   Question平均长度: {statistics.mean(question_lengths):.1f} 字符 (包含空值)")
        if valid_question_lengths:
            print(f"   Question平均长度: {statistics.mean(valid_question_lengths):.1f} 字符 (仅非空值)")
        ...
        if valid_question_lengths:
            print(f"   Question长度范围: {min(valid_question_lengths)} - {max(valid_question_lengths)} 字符 (仅非空值)")
...
def analyze_tatqa_converted_data():
    ...
    try:
        ...
        # 统计answer_from分布（使用整个数据集：训练集+评估集）
        answer_from_stats = {}
        all_data = train_converted + eval_converted
        
        for item in all_data:
            answer_from = item.get('answer_from', 'unknown')
            answer_from_stats[answer_from] = answer_from_stats.get(answer_from, 0) + 1
        
        print(f"\n📋 答案来源分布 (整个数据集 - 训练集+评估集):")
        for source, count in answer_from_stats.items():
            percentage = count / len(all_data) * 100
            print(f"   {source}: {count:,} ({percentage:.1f}%)")
        
        # 长度统计（使用整个数据集）
        context_lengths = [len(item.get('context', '')) for item in all_data]
        answer_lengths = [len(item.get('answer', '')) for item in all_data]
        question_lengths = [len(item.get('query', '')) for item in all_data]
        ...
        # 元数据覆盖率（使用整个数据集）
        doc_id_coverage = sum(1 for item in all_data if item.get('doc_id')) / len(all_data) * 100
        relevant_doc_coverage = sum(1 for item in all_data if item.get('relevant_doc_ids')) / len(all_data) * 100
