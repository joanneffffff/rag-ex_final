...
def _shared_text_standardizer(text: str) -> str:
    ...
    # <<< 关键修改 >>> 移除 final answer is 和 final number is
    text = re.sub(r'^(the\s*answer\s*is|it\s*was|the\s*value\s*is|resulting\s*in|this\s*represents|the\s*effect\s*is|therefore|so|thus|in\s*conclusion)\s*', '', text, flags=re.IGNORECASE).strip()
...
def extract_final_answer_with_rescue(raw_output: str) -> str:
    """
    从模型的原始输出中提取最终答案。
    只寻找 <answer> 标签。如果失败或未找到，则返回空字符串。
    """
    ...
    if match:
        ...
        # 确保提取的内容非空，且不是一个空标签对本身（防止模型生成 <answer></answer>）
    ...
    # 如果没有找到精确的 <answer>...</answer> 结构，返回空字符串
    # 这意味着模型没有遵循指令，答案提取失败
    return ""
